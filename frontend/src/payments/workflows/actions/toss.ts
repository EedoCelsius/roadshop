...
const ensureTossDeepLink = async (context: PaymentActionContext): Promise<string | null> => {
  const loaded = await context.ensureMethodInfoLoaded('toss')
...
const runTossWorkflow = async (context: PaymentActionContext) => {
  ...
  try {
    await launchDeepLink(deepLink, {
      timeoutMs: 2000,
      onNotInstalled: () => context.showDeepLinkDialog('not-installed', 'toss'),
      onNotMobile: () => context.showDeepLinkDialog('not-mobile', 'toss', { deepLinkUrl: deepLink }),
    })
  } finally {
    context.completeTossInstructionDialog()
  }
